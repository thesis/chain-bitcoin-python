...
import sure
from .. import Chain, NoApiKeyId, Block, get_block_by_hash
from .mock_http_adapter import *


def test_get_block_by_hash():
    get_block_by_hash(block_hash, api_key_id=api_key_id,
                      http_adapter=http_adapter) \
        .should.equal(block)


def test_get_block_by_hash_using_class():
    Chain(api_key_id=api_key_id, http_adapter=http_adapter) \
        .get_block_by_hash(block_hash).should.equal(block)


def test_get_block_by_hash_without_api_key_id():
    (lambda: get_block_by_hash(block_hash, http_adapter=no_http())) \
