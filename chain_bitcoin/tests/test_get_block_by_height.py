...
def test_get_block_by_height():
    get_block_by_height(block_height, api_key_id=api_key_id,
...
def test_get_block_by_height_using_class():
    Chain(api_key_id=api_key_id, http_adapter=http_adapter) \
        .get_block_by_height(block_height).should.equal(block)


def test_get_block_by_height_without_api_key_id():
    (lambda: get_block_by_height(block_height, http_adapter=no_http())) \
...
url = 'https://api.chain.com/v1/bitcoin/blocks/308920?api-key-id=DEMO-4a5e1e4'
